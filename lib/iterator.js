...
Iterator.prototype._open = function (cb) {
  ...
  this._db.get(this._prefix, opts, function (err, heads) {
    ...
    console.log('PREFIX HASH:', hash(self._prefix, false))
    // Subtract the separator.
    self._start = prefixLength - 1
...
function next (iterator, worker, cb) {
  ...
  console.log('HERE', 'node.path:', node.path.join(''), 'node.trie:', JSON.stringify(node.trie), 'trie length:', node.trie.length)
  for (var i = top.i; i < node.trie.length; i++) {
    ...
    var sortEnd = gt && iterator._start === i ? hash.SEPARATE : hash.TERMINATE

    console.log('sortEnd:', sortEnd, 'i:', i, 'iterator._start:', iterator._start)
    for (var j = 0; j < sortEnd; j++) {
      var sortValue = j === hash.TERMINATE ? hash.TERMINATE : hash.SEPARATE - j // 4, 3, 2, 1, 0, 5
      console.log('i:', i, 'j:', j, 'sortValue:', sortValue, 'node.path[i]:', node.path[i], 'bucket:', bucket[sortValue])
      ...
      if (sortValue === node.path[i]) {
        ...
        if (sortValue !== hash.TERMINATE) continue
      }
    ...
    if (!iterator._recursive && node.path[i] === hash.SEPARATE)
      break
