...
Iterator.prototype._consume = function (cb) {
  ...
  this._mapWorkers((w, next) => {
    ...
    var selected = filter(select(nodes))
    if (!selected.length) return cb(null, null)
    return cb(null, self._prereturn(selected))
  })
  ...
  // Group together all candidates that share the same key, and
  // remove the values from the workers who proposed them.
  // (This second step allows the workers to continue searching).
  ...
  function filter (nodes) {
    if (nodes.length > 1) nodes = filterHeads(nodes)
    if (self._gt && nodes[0].key === self._prefix) nodes = []
    return nodes
  }
}
...
Worker.prototype.search = function (cb) {
  ...
  function insert (nodes) {
    for (var i = nodes.length - 1; i >= 0; i--) {
      self.stack.push({ nodes: nodes[i], i: next.i + 1 })
    }
  }
}
...
Worker.prototype._findNewEntries = function (entry, cb) {
  ...
  function insert (idx, nodes) {
    console.log('IN INSERT, idx:', idx, 'NODES:', nodes, 'GT:', self._iterator._gt, 'ENTRY:', entry)
...
/**
 * The default search algorithm might propose candidates for a given key
 * that are not the head values for that key.
 *
 * Since the proposed set also must contain all head values, we can filter
 * out the non-heads.
 *
 * This is n^2 in the number of forks.
 */
function filterHeads (nodes) {
  return nodes.filter(n => {
    for (var i = 0; i < nodes.length; i++) {
      console.log('COMPARING:', nodes[i], 'AND', n, 'C1:', nodes[i].clock, 'C2:', n.clock)
      if (nodes[i] !== n && largerClock(nodes[i].clock, n.clock)) {
        return false
      }
    }
    console.log('FILTER HEADS DIDNt fILTER')
    return true
  })
}
...
function maxClock (c, node) {
  if (!c) return node.clock
  if (largerClock(node.clock, c)) return node.clock
  return c
}
