  ...
  this._db.get(this._prefix, opts, function (err, heads) {
    ...
    if (heads.length > 1) {
      ...
      for (var i = 0; i < heads.length; i++) {
        self._createWorker(heads[i], self._offset, forks[i])
      }
    } else {
      self._createWorker(heads[0], self._offset, null)
    }
    return cb()
  })
}
...
Iterator.prototype._next = function (cb) {
  ...
  function select (nodes) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (nodes[i][0].key === minKey) continue
      break
    }
...
Worker.prototype.search = function (cb) {
  ...
  if (node && this._outsideRange(node.clock)) {
    ...
    console.log('outside clock')
    return this.search(cb)
  }
  if (next.i < this._end && trie.length < next.i) {
  ...
  function done (err, newEntries) {
    ...
    if (!newEntries.lt.length) {
      console.log('SETTING VALUE TO:', next.nodes)
