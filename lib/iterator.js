...
Iterator.prototype._push = function (nodes, i, allowConflict) {
  ...
  if (nodes.length > 1 && !allowConflict) {
    var trie = mergeTries(nodes)
    var entries = [{ trie, i, nodes }]
  } else {
...
Iterator.prototype._loadNodes = function (bucket, cb) {
  ...
  function done(err, node) {
    ...
    console.log('remaining:', remaining, 'bucket:', bucket, 'node:', node)
  ...
  function done (release) {
    ...
    if (--remaining <= 0) {
      ...
      for (var j = nodesByPath.length - 1; j >= 0; j--) {
        ...
        self._push(toPush, allowConflict ? i + 1 : i, allowConflict)
      }
      release()
...
function mergeTries (nodes) {
  ...
  console.log('MERGED TRIE:', JSON.stringify(mergedTrie))
  return mergedTrie
}
