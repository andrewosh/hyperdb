...
module.exports = PrefixIterator

function PrefixIterator (db, prefix, opts) {
  if (!(this instanceof PrefixIterator)) return new PrefixIterator(db, prefix, opts)
  if (!opts) opts = {}
...
inherits(PrefixIterator, nanoiterator)

PrefixIterator.prototype._pushPointer = function (ptr, i, cb) {
  var self = this
...
PrefixIterator.prototype._pushNode = function (node, i) {
  this._stack.push({
    path: null,
    node,
    i
...
PrefixIterator.prototype._pushPrefix = function (path, i, val) {
  this._stack.push({
...
// fast case
PrefixIterator.prototype._singleNode = function (top, cb) {
  var node = top.node

  for (var i = top.i; i < node.trie.length; i++) {
    if (!this._recursive && node.path[i] === path.SEPARATE) break
...
// slow case
PrefixIterator.prototype._multiNode = function (path, nodes, cb) {
...
PrefixIterator.prototype._filterResult = function (nodes, i) {
  var result = null
...
PrefixIterator.prototype._lookupPrefix = function (path, cb) {
  var self = this
...
PrefixIterator.prototype._prereturn = function (nodes) {
  if (this._map) nodes = nodes.map(this._map)
  if (this._reduce) return nodes.reduce(this._reduce)
  return nodes
}

PrefixIterator.prototype._sortOrder = function (i) {
