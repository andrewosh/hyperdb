...
LexIterator.prototype._next = function (cb) {
  ...
  if (this._count >= this._limit) return cb(null, null)
  ...
  function onFinish () {
    ...
    // Filter out non-heads and dupes from the final result.
    // TODO: is this acceptable?
    if (result.length > 1) {
      result = filterHeads(removeDuplicates(result))
    }
...
Worker.prototype._searchTrie = function (node, index, gtSafe, ltSafe) {
  ...
  for (var i = index; i < node.path.length; i++) {
    ...
    for (var j = 0; j < SORT_ORDER.length; j++) {
      ...
      if (bucket && bucket[idx] &&
        ...
        var slot = (SORT_BOUNDS[idx] < SORT_BOUNDS[pathVal]) ? ltSlot : gtSlot
        slot.unshift({
          ptr: bucket[idx],
          i,
    ...
    gtCandidates.push.apply(gtCandidates, gtSlot)
    ltCandidates.unshift.apply(ltCandidates, ltSlot)
  ...
  var top = this._reverse ? ltCandidates : gtCandidates
  var bottom = this._reverse ? gtCandidates : ltCandidates
  if (this._reverse) {
    top.reverse()
    bottom.reverse()
  }

  this._stack.push.apply(this._stack, top)
  if (nodeInBounds) this._stack.push({ node })
  this._stack.push.apply(this._stack, bottom)
...
/*
 ...
 1) For each head, do a pairwise calculation across all other heads
    to find its last common ancestors.
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    for (var j = 0; j < candidates.length; j++) {
      ...
      if (c1 === c2) {
        ...
        var lca = lastCommonAncestor(c1, c2)
...
function lastCommonAncestor (c1, c2) {
...
// TODO: wish we didn't need this.
function removeDuplicates (list) {
  var nodes = {}
  var result = []
  for (var i = 0; i < list.length; i++) {
    var node = list[i]
    if (nodes[node.seq] && nodes[node.seq][node.feed]) continue
    if (nodes[node.seq]) {
      nodes[node.seq][node.feed] = true
      result.push(node)
    } else {
      nodes[node.seq] = {}
      nodes[node.seq][node.feed] = true
