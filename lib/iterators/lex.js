...
LexIterator.prototype._next = function (cb) {
  ...
  function onFinish () {
    ...
    if (result.length > 1) {
      console.log('FILTERING HEADS FROM:', result.map(n => n.clock))
...
LexIterator.prototype._prereturn = function (nodes) {
  ...
  console.log('ITE, RETURNING:', nodes, 'clocks:', nodes.map(n => n.clock))
  return nodes
}
...
Worker.prototype._outsideRange = function (clock) {
  ...
  console.log('CHECKING CLOCKS:', clock, 'this._fork:', this._fork)
...
Worker.prototype.search = function (cb) {
  ...
  function searchAndFinish () {
    ...
    console.log('top.node.clock:', top.node.clock)
