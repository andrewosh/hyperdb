...
LexIterator.prototype._next = function (cb) {
  ...
  function onFinish () {
    ...
    if (result[0].key === self._lt) return cb(null, null)
...
Worker.prototype._searchTrie = function (node, index) {
  ...
  for (var i = index; i < node.trie.length; i++) {
    ...
    console.log('  ITE, pathVal: ', pathVal, 'node.path:', node.path, 'bucket:', bucket, 'upper:', upper, 'lower:', lower)
    for (var j = 0; j < SORT_BOUNDS[pathVal]; j++) {
  ...
  for (i = index; i < node.trie.length; i++) {
    ...
    console.log('  ITE, pathVal: ', pathVal, 'node.path:', node.path, 'bucket:', bucket, 'upper:', upper, 'lower:', lower)
    for (j = SORT_BOUNDS[pathVal]; j < SORT_ORDER.length; j++) {
      idx = SORT_ORDER[j]
  ...
  function outOfBounds (val, upper, lower) {
    ...
    return (((upper !== undefined) && SORT_BOUNDS[val] > SORT_BOUNDS[upper]) ||
